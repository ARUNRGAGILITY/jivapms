...
def list_roles_of_user(app_label='app_memberprofilerole', model_name='Role', role_name='Scrum Master'):
    try:
        ...
        get_role =  model.objects.filter(name=role_name, active=True).first()
        get_members = MemberOrganizationRole.objects.filter(role=get_role, active=True)
        return get_members
...
def admin_dashboard(request):
    ...
    scrum_masters = list_roles_of_user(role_name='Scrum Master').count()
    product_owners = list_roles_of_user(role_name='Product Owner').count()
    team_members =list_roles_of_user(role_name='TeamMember').count()
            ...
            try:
                ...
                for i, row in enumerate(csv_reader, 1):
                    ...
                    try:
                        ...
                        # # Validate user role
                        role_name = row_data.get('role', 'general').strip()
